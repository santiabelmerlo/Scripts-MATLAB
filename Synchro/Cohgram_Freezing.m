%% Coherence for BLA, PL and IL during freezing and no freezing events
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clc
clear all
    ...
    for d = 1:length(D_folders)
        ...
        if exist(strcat(name,'_sessioninfo.mat')) == 2;
            ...
            if exist(strcat(name, '_lfp.dat')) && ...
                ...
                i = 1;
                while i <= n;
                    random_time = randi([120, round(t(end)-60)]);
                ...
                pos1 = fin_freezing > t(end) - 120; % Aquellos eventos de freezing que terminen dentro de los últimos 10 seg de señal
                pos2 = fin_nofreezing > t(end) - 120; % Aquellos eventos de no freezing que terminen dentro de los últimos 10 seg de señal
                ...
                % Elimino los eventos de freezing que estan muy cerca del comienzo
                clear pos1
                pos1 = inicio_freezing < 30;
                inicio_freezing(pos1) = [];
                fin_freezing(pos1) = [];
                duracion_freezing(pos1) = [];
                ...
                if exist('lfp_BLA') && exist('lfp_PL') && exist('lfp_IL')
                    ...
                    % Busco las posiciones en S donde inician y finalizan los eventos de freezing y no freezing
                    clear CS1_inicioenS CS1_finenS CS2_inicioenS CS2_finenS
                    j = 1;
                    for i = 1:size(inicio_freezing,2);
                        CS1_inicioenS(j) = min(find(abs(t-inicio_freezing(i)) == min(abs(t-inicio_freezing(i)))));
                        CS1_finenS(j) = min(find(abs(t-fin_freezing(i)) == min(abs(t-fin_freezing(i)))));
                        j = j + 1;
                    end
                    j = 1;
                    for i = 1:size(inicio_nofreezing,2);
                        CS2_inicioenS(j) = min(find(abs(t-inicio_nofreezing(i)) == min(abs(t-inicio_nofreezing(i)))));
                        CS2_finenS(j) = min(find(abs(t-fin_nofreezing(i)) == min(abs(t-fin_nofreezing(i)))));
                        j = j + 1;
                    end
                    ...
                    elseif strcmp(paradigm_toinclude,'aversive');
                        window = 40; % Porque dentro del tono son 120 ventanas de 0.5 seg.

                        j = 1;
                        for i = 1:size(CS1_inicioenS,2);
                            C1_CS1(:,:,j) = C_1(CS1_inicioenS(1,i)-(window):CS1_inicioenS(1,i)+(window)-1,:);
                            C2_CS1(:,:,j) = C_2(CS1_inicioenS(1,i)-(window):CS1_inicioenS(1,i)+(window)-1,:);
                            SC1_CS1(:,:,j) = S12_1(CS1_inicioenS(1,i)-(window):CS1_inicioenS(1,i)+(window)-1,:);
                            SC2_CS1(:,:,j) = S12_2(CS1_inicioenS(1,i)-(window):CS1_inicioenS(1,i)+(window)-1,:);
                            j = j + 1;
                        end
                        
                        j = 1;
                        for i = 1:size(CS2_inicioenS,2);
                            C1_CS2(:,:,j) = C_1(CS2_inicioenS(1,i)-(window):CS2_inicioenS(1,i)+(window)-1,:);
                            C2_CS2(:,:,j) = C_2(CS2_inicioenS(1,i)-(window):CS2_inicioenS(1,i)+(window)-1,:);
                            SC1_CS2(:,:,j) = S12_1(CS2_inicioenS(1,i)-(window):CS2_inicioenS(1,i)+(window)-1,:);
                            SC2_CS2(:,:,j) = S12_2(CS2_inicioenS(1,i)-(window):CS2_inicioenS(1,i)+(window)-1,:);
                            j = j + 1;
                        end
                        
                        if size(C1_CS1,2) == 1967 && size(C1_CS1,1) == 80;
...
% Curva de coherencia
figure()
plot_curve(f,(squeeze(nanmean(C2_CS2_cat(41:51,:,:),1)))', 'median', nofz_color, 200,'dis'); hold on;
plot_curve(f,(squeeze(nanmean(C2_CS1_cat(41:51,:,:),1)))', 'median', fz_color, 200,'dis'); hold on;
plot_curve(f,(squeeze(nanmean(C1_CS2_cat(41:51,:,:),1)))', 'median', nofz_color, 200,'cont'); hold on;
plot_curve(f,(squeeze(nanmean(C1_CS1_cat(41:51,:,:),1)))', 'median', fz_color, 200,'cont'); hold on;
hCurve = findobj(gca, 'Type', 'Line');
legend(hCurve, {'BLA-PL; Fz','BLA-PL; noFz','BLA-IL; Fz','BLA-IL; noFz'},'Location','NorthEastOutside');
...
figure()
subplot(151)
plot_coh(C1_CS1_cat,C1_CS2_cat,C2_CS1_cat,C2_CS2_cat,f,1,2);
subplot(152)
plot_coh(C1_CS1_cat,C1_CS2_cat,C2_CS1_cat,C2_CS2_cat,f,2,2);
subplot(153)
plot_coh(C1_CS1_cat,C1_CS2_cat,C2_CS1_cat,C2_CS2_cat,f,3,2);
subplot(154)
plot_coh(C1_CS1_cat,C1_CS2_cat,C2_CS1_cat,C2_CS2_cat,f,4,2);
subplot(155)
plot_coh(C1_CS1_cat,C1_CS2_cat,C2_CS1_cat,C2_CS2_cat,f,5,2);
