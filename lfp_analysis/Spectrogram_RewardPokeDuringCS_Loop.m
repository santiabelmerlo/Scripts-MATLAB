...
% Espectrograma centrado al reward poke que ocurre durante el tono CS, 20 segundos antes y 20 segundos después
                    ...
                    if strcmp(paradigm_toinclude,'appetitive');
                        ...
                        if CS1_inicioenS(1,end) < size(S,1) && round(mean(CS1_finenS - CS1_inicioenS)) == 20;
                            for i = 1:size(CS1_inicioenS,2);
                                if any(head_entries_duringCS(:,i) == 1);
                                    pos = unique(find(head_entries_duringCS(:,i) == 1))-1;
...
ylabel(['Frequency (Hz)']);
xlabel('Time (sec.)');
title(strcat(region,' Reward Poke During CS+', 'n= ', num2str(n1)));
colormap(ax1,jet);    
...
ylabel(['Frequency (Hz)']);
xlabel('Time (sec.)');
title(strcat(region,' Reward Poke During CS-', 'n= ', num2str(n2)));
colormap(ax2,jet);    
