%% Script para calcular la envolvente de la potencia, y luego normalizada
% Lo calcula mediante la transformada de hilbert
% Luego cuantifica dentro del CS o en el periodo determinado
% Plotea la curva de normalized power envelope en función de los trials y
% un boxplot con una cuantificación promedio de la sesión
clc;
clear all;
...
meantitle = 'Ext1 BLA Spectrogram'; % Titulo general que le voy a poner a la figura
% region = 'BLA'; % Región que quiero analizar: BLA, PL, IL.
region = 'PL'; % Región que quiero analizar: BLA, PL, IL.
...
% freqpass = [2 5.3]; % Rango de frecuencias que voy a analizar
freqpass = [5.3 9.6]; % Rango de frecuencias que voy a analizar
% freqpass = [13 30]; % Rango de frecuencias que voy a analizar
% freqpass = [43 60]; % Rango de frecuencias que voy a analizar
% freqpass = [60 98]; % Rango de frecuencias que voy a analizar


% freqpass = [102 150]; % Rango de frecuencias que voy a analizar
...
subplot(1,5,4);
plot_power_boxplot(SPGG_CS1(1:10,:),SPGG_CS2(1:10,:),region,paradigm_toinclude);
title('Early Session');
subplot(1,5,5);
plot_power_boxplot(SPGG_CS1(51:60,:),SPGG_CS2(51:60,:),region,paradigm_toinclude);
title('Late Session');
