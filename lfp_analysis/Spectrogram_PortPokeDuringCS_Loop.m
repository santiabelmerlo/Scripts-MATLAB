...
% Espectrograma centrado al port poke que ocurre durante el tono CS, 20 segundos antes y 20 segundos después
...
trials_toinclude = 41:60; % Filtro por los trials que quiero incluir de la sesión
meantitle = 'Early Extinction 2 IL Spectrogram'; % Titulo general que le voy a poner a la figura
                    ...
                    if strcmp(paradigm_toinclude,'appetitive');
                        ...
                        if CS1_inicioenS(1,end) < size(S,1) && round(mean(CS1_finenS - CS1_inicioenS)) == 20;
                            for i = 1:size(CS1_inicioenS,2);
                                if any(nosepoke_duringCS(:,i) == 1);
                                    pos = unique(find(nosepoke_duringCS(:,i) == 1))-1;
                                    S_CS1(:,:,i) = S(CS1_inicioenS(1,i)+pos-2*window:CS1_inicioenS(1,i)+pos+2*window-1,:);
                                    S_CS2(:,:,i) = S(CS2_inicioenS(1,i)+pos-2*window:CS2_inicioenS(1,i)+pos+2*window-1,:);
                                else
...
ylabel(['Frequency (Hz)']);
xlabel('Time (sec.)');
title(strcat(region,' Port Poke During CS+', 'n= ', num2str(n1)));
colormap(ax1,jet);    
...
ylabel(['Frequency (Hz)']);
xlabel('Time (sec.)');
title(strcat(region,' Port Poke During CS-', 'n= ', num2str(n2)));
colormap(ax2,jet);    
...
ylabel(['Frequency (Hz)']);
xlabel('Time (sec.)');
title(strcat(region,' Port Poke Delta CS'));
