                    ...
                    if strcmp(paradigm_toinclude,'appetitive');
                        ...
                        if CS1_inicioenS(1,end) < size(S,1) && round(mean(CS1_finenS - CS1_inicioenS)) == 20;
                            for i = 1:size(CS1_finenS,2);
                                if any(nosepoke_postCS(:,i) == 1);
                                    pos = unique(find(nosepoke_postCS(:,i) == 1))-1;
...
ylabel(['Frequency (Hz)']);
xlabel('Time (sec.)');
title(strcat(region,' Port Poke Post CS+', 'n= ', num2str(n1)));
colormap(ax1,jet);    
...
ylabel(['Frequency (Hz)']);
xlabel('Time (sec.)');
title(strcat(region,' Port Poke Post CS-', 'n= ', num2str(n2)));
colormap(ax2,jet);    
