...
% Espectrograma centrado al port poke que ocurre luego del offset del tono CS, 20 segundos antes y 20 segundos después
...
% rats = [10,11,14,16,17,18,19]; % Filtro por animales para apetitivo cuando el animal 13 daba out of index
...
meantitle = 'Extinction 1 IL Spectrogram'; % Titulo general que le voy a poner a la figura
...
remove_epileptic = 0; % 1 para limpiar los momentos epilépticos de la señal, 0 para no limpiar
        ...
        if exist(strcat(name,'_sessioninfo.mat')) == 2;
            ...
            if strcmp(paradigm,paradigm_toinclude) && (any(strcmp(session, session_toinclude))||any(strcmp(session_end, session_toinclude)));
                ...
                if exist(strcat(name,'_sessioninfo.mat')) == 2 && ...
                    ...
                    % Cargamos behavior_timeseries.mat head_entries y nosepoke
                    % time_step = 0.5
                    load(strcat(name,'_behavior_timeseries.mat'),'head_entries','nosepoke','tt');
                    onset = find(tt == 0);
                    offset = find(tt == 10);
                    head_entries_duringCS = head_entries(onset:offset,:);
                    nosepoke_duringCS = nosepoke(onset:offset,:);
                    head_entries_postCS = head_entries(offset:end,:);
                    nosepoke_postCS = nosepoke(offset:end,:);
                    
                    head_entries_duringCS = diff(head_entries_duringCS,1,1) == 1;
                    nosepoke_duringCS = diff(nosepoke_duringCS,1,1) == 1;
                    head_entries_postCS = diff(head_entries_postCS,1,1) == 1;
                    nosepoke_postCS = diff(nosepoke_postCS,1,1) == 1;
                    ...
                    if strcmp(paradigm_toinclude,'appetitive');
                        ...
                        if CS1_inicioenS(1,end) < size(S,1) && round(mean(CS1_finenS - CS1_inicioenS)) == 20;
                            for i = 1:size(CS1_finenS,2);
                                if any(head_entries_postCS(:,i) == 1);
                                    pos = unique(find(head_entries_postCS(:,i) == 1))-1;
                                    S_CS1(:,:,i) = S(CS1_finenS(1,i)+pos-2*window:CS1_finenS(1,i)+pos+2*window-1,:);
                                    S_CS2(:,:,i) = S(CS2_finenS(1,i)+pos-2*window:CS2_finenS(1,i)+pos+2*window-1,:);
                                else
                                    S_CS1(:,:,i) = nan(window*4,size(f,2));
                                    S_CS2(:,:,i) = nan(window*4,size(f,2));
                                end
                            end
                        else 
                            for j = 1:size(CS1_finenS,2);
                                S_CS1(:,:,j) = nan(window*4,size(f,2));
                                S_CS2(:,:,j) = nan(window*4,size(f,2));
                            end
                        end
                        
                        if size(S_CS1,2) == 1967 && size(S_CS1,1) == 80;
...
% Calculamos cuantos n tengo de cada uno
n1 = sum(squeeze(mean(mean(~isnan(SPG_CS1), 1), 2)) > 0);
n2 = sum(squeeze(mean(mean(~isnan(SPG_CS2), 1), 2)) > 0);
...
ylabel(['Frequency (Hz)']);
xlabel('Time (sec.)');
title(strcat(region,' Reward Poke Post CS+', 'n= ', num2str(n1)));
colormap(ax1,jet);    
...
ylabel(['Frequency (Hz)']);
xlabel('Time (sec.)');
title(strcat(region,' Reward Poke Post CS-', 'n= ', num2str(n2)));
colormap(ax2,jet);    
...
ylabel(['Frequency (Hz)']);
xlabel('Time (sec.)');
title(strcat(region,' Reward Poke Delta CS'));
