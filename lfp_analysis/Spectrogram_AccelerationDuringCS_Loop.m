...
% Espectrograma centrado al momento de aceleracion o desaceleracion que ocurre durante el tono CS, 20 segundos antes y 20 segundos después
        ...
        if exist(strcat(name,'_sessioninfo.mat')) == 2;
            ...
            if strcmp(paradigm,paradigm_toinclude) && (any(strcmp(session, session_toinclude))||any(strcmp(session_end, session_toinclude)));
                ...
                if exist(strcat(name,'_sessioninfo.mat')) == 2 && ...
                        ...
                        exist(strcat(name,'_acceleration.mat')) == 2 && ...
                    ...
                    % Cargamos R00D00_acceleration.mat aceleracion y desaceleracion
                    load(strcat(name,'_acceleration.mat'),'aceleracion','desaceleracion');
                    ...
                    elseif strcmp(paradigm_toinclude,'aversive');
                        ...
                        if CS1_inicioenS(1,end) < size(S,1) && round(mean(CS1_finenS - CS1_inicioenS)) == 120;
                            for i = 1:size(CS1_inicioenS,2);
                                if any(desaceleracion >= TTL_CS1_inicio(i) & desaceleracion < TTL_CS1_fin(i));
                                    pos = min(find(desaceleracion(1,:) - TTL_CS1_inicio(i) > 0));
                                    pos_S = round((desaceleracion(1,pos) - TTL_CS1_inicio(i))/0.5);
                                    S_CS1(:,:,i) = S(CS1_inicioenS(1,i)+pos_S-(window):CS1_inicioenS(1,i)+pos_S+(window)-1,:);
                                else
                                    S_CS1(:,:,i) = nan(window*2,size(f,2));                                   
                                end  
                            end
                            for i = 1:size(CS2_inicioenS,2);
                                if any(desaceleracion >= TTL_CS2_inicio(i) & desaceleracion < TTL_CS2_fin(i));
                                    pos = min(find(desaceleracion(1,:) - TTL_CS2_inicio(i) > 0));
                                    pos_S = round((desaceleracion(1,pos) - TTL_CS2_inicio(i))/0.5);
                                    S_CS2(:,:,i) = S(CS2_inicioenS(1,i)+pos_S-(window):CS2_inicioenS(1,i)+pos_S+(window)-1,:);
                                else
...
ylabel(['Frequency (Hz)']);
xlabel('Time (sec.)');
title(strcat(region,' Desacceleration Onset During CS+'));
colormap(ax1,jet);    
...
ylabel(['Frequency (Hz)']);
xlabel('Time (sec.)');
title(strcat(region,' Desacceleration Onset Delta CS'));
