...
%         end 
        % draw whiskers
        if confs.whiskers == 1
            % Plot the vertical parts of the whiskers (remove the horizontal hat parts)
            h.wh(k,g,:) = plot(whi_xcor(:,k), whi_ycor(:,1,k),'k-', ...  % Left whisker
                             whi_xcor(:,k), whi_ycor(:,2,k),'k-', ...  % Right whisker
                             'LineWidth', 1);
        end
