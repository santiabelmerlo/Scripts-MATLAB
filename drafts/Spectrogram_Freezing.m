...
%                     duracion_freezing = duracion_freezing(duracion_freezing >=3);
                    ...
                    % Filtramos por momento del freezing: CS+, CS- o preCS
                    inicio_freezing = inicio_freezing(freezing_type == 1);
                    fin_freezing = fin_freezing(freezing_type == 1);
                    duracion_freezing = duracion_freezing(freezing_type == 1);
                    
                    % Filtramos por los freezing que estan en la primera
                    % mitad de la sesión vs la segunda mitad
                    inicio_freezing = inicio_freezing(inicio_freezing >= TTL_CS1_inicio(11));
                    fin_freezing = fin_freezing(inicio_freezing >= TTL_CS1_inicio(11));
                    duracion_freezing = duracion_freezing(inicio_freezing >= TTL_CS1_inicio(11));
                    
                    inicio_freezing = inicio_freezing(inicio_freezing <= TTL_CS1_fin(20));
                    fin_freezing = fin_freezing(inicio_freezing <= TTL_CS1_fin(20));
                    duracion_freezing = duracion_freezing(inicio_freezing <= TTL_CS1_fin(20));
                    
                    duracion = cat(2,duracion,duracion_freezing);
...
val_t_post = [0 3];
val_t_pre = [-3 0]; 
val_f = [2 4]; freq_label = '4-Hz'; % 4-Hz
% val_f = [5.3 9.6]; freq_label = 'Theta'; % Theta
% val_f = [43 60]; freq_label = 'sGamma'; % sGamma
% val_f = [60 98]; freq_label = 'fGamma'; %fGamma
