...
classify = 'BLA'; % Región cerebral que usamos para clasificar los eventos de freezing
...
% enriched
if strcmp(classify,'BLA');
    PowerSheet.FzType = NaN(size(PowerSheet.FourHz_BLA));
    for i = 1:size(PowerSheet.FourHz_BLA)
        if ~isnan(PowerSheet.FourHz_BLA(i)) && ~isnan(PowerSheet.Theta_BLA(i))
            PowerSheet.FzType(i) = ~(PowerSheet.FourHz_BLA(i) >= PowerSheet.Theta_BLA(i));
        end
    end
elseif strcmp(classify,'PL');
    PowerSheet.FzType = NaN(size(PowerSheet.FourHz_PL));
    for i = 1:size(PowerSheet.FourHz_PL)
        if ~isnan(PowerSheet.FourHz_PL(i)) && ~isnan(PowerSheet.Theta_PL(i))
            PowerSheet.FzType(i) = ~(PowerSheet.FourHz_PL(i) >= PowerSheet.Theta_PL(i));
        end
    end
elseif strcmp(classify,'IL'); 
    PowerSheet.FzType = NaN(size(PowerSheet.FourHz_IL));
    for i = 1:size(PowerSheet.FourHz_IL)
        if ~isnan(PowerSheet.FourHz_IL(i)) && ~isnan(PowerSheet.Theta_IL(i))
            PowerSheet.FzType(i) = ~(PowerSheet.FourHz_IL(i) >= PowerSheet.Theta_IL(i));
        end
    end
end
...
PowerSheet = table(PowerSheet.ID,PowerSheet.(column),PowerSheet.FzType,'VariableNames',{'ID','Power','FzType'});
    ...
    for d = 1:length(D_folders)
        ...
        if exist(strcat(name,'_sessioninfo.mat')) == 2;
            ...
            if exist(strcat(name,'_specgram_',region,'LowFreq.mat')) == 2 && ...
               ...
               event_fztype = MergedTable.FzType;
               ...
               if ~isempty(event_inicio);
                   ...
                   S1 = zscorem(S,1);
                    ...
                    data = table(event_ID, event_type, Sstack, event_fztype,'VariableNames',{'ID','Type','S','FzType'});
                   ...
                   PowerData = vertcat(PowerData,MergedTable);
                    ...
                    if exist(strcat(name,'_behavior_timeseries.mat')) == 2
                        ...
                        BehaviorSeries = table(event_ID, event_type, event_fztype, behavior_segments, ...
                                               'VariableNames', {'ID', 'Type', 'FzType','Acc'});
...
h = boxplot(PowerData.Power, PowerData.FzType, ...
    'color', lines, ...
...
CS_plus = PowerData.Power(PowerData.FzType == 0);
CS_minus = PowerData.Power(PowerData.FzType == 1);
...
figure; % Create a new figure
h = boxplot(PowerData.Duracion, PowerData.FzType, ...
    'color', lines, ...
    'labels', {'4Hz-Fz', 'Theta-Fz'}, ...
    'symbol', '', ...
    'whisker', 1, ...
...
% Extract data for each condition
CS_plus = PowerData.Duracion(PowerData.FzType == 0);
CS_minus = PowerData.Duracion(PowerData.FzType == 1);
...
uniqueTypes = unique(SpecData.FzType);  % Get unique values in 'Type' column
...
for i = 1:length(uniqueTypes)
    ...
    currentTypeRows = SpecData.FzType == uniqueTypes(i); 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Behavior Timeseries
BehaviorTimeSeries = BehaviorTimeSeries(~isnan(BehaviorTimeSeries.FzType), :);

% Define custom labels for each Type
type_labels = containers.Map([0, 1], {'4Hz Fz', 'Theta Fz'});

% Get unique Types after merging
unique_types = unique(BehaviorTimeSeries.FzType);
...
for i = 1:length(unique_types)
    ...
    type_rows = BehaviorTimeSeries.FzType == unique_types(i);
