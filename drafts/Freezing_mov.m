...
clc;
% clear all;
clearvars -except Fz_fourhz Fz_Theta Mov_fourhz Mov_theta
            ...
            if strcmp(paradigm,paradigm_toinclude) && (any(strcmp(session, session_toinclude))||any(strcmp(session_end, session_toinclude)));
                ...
                if exist(strcat(name,'_sessioninfo.mat')) == 2 && ...
                    ...
                    if exist(strcat(name,'_behavior_timeseries.mat')) == 2
                        load(strcat(name,'_behavior_timeseries.mat'), 'behavior_timeseries','time_vector'); % Número de canales totales
                        for i = 1:length(inicio_freezing)-10 
                            fz_inicio = min(find(abs(time_vector-inicio_freezing(1,i)) == min(abs(time_vector-inicio_freezing(1,i)))));
                            if fz_inicio >= 52
                                mov_fz = cat(2,mov_fz,behavior_timeseries(fz_inicio-51:fz_inicio+49,7));
                                clear fz_inicio
                            end
                        end
                        for i = 1:length(inicio_movement)-10 
                            fz_inicio = min(find(abs(time_vector-inicio_movement(1,i)) == min(abs(time_vector-inicio_movement(1,i)))));
                            if fz_inicio >= 52
                                mov_nofz = cat(2,mov_nofz,behavior_timeseries(fz_inicio-51:fz_inicio+49,7));
                                clear fz_inicio
                            end
                        end
                        clear behavior_timeseries time_vector inicio_nofreezing
                    end
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
...
elseif frange == 2; val_f = [5.3 9.6]; freq_label = 'Theta'; % Theta
elseif frange == 3; val_f = [13 30]; freq_label = 'Beta'; % Beta
elseif frange == 4; val_f = [46 54]; freq_label = 'sGamma'; % sGamma [43 60] Rango original
...
subplot(121)
    behaviour_color = [10 10 10]/255; % Seteo el color para comportamiento   
    S_data = mov_fz';
    y = smooth(nanmean(S_data)); % your mean vector;
    x = -25:0.5:25;
    ...
    p1 = fill(x2, inBetween, behaviour_color,'LineStyle','none');
    set(p1,'facealpha',.4)
    hold on;
    plot(x, y, 'Color',behaviour_color, 'LineWidth', 1);
    hold on;
    clear S_data;
    ylim([5 25]);
    xlim([-5 5]);
    line([0 0],[0 50],'Color',behaviour_color,'LineWidth',0.5,'LineStyle','--');
    xlabel('Time (sec.)'); ylabel('Acceleration (cm/s2)');
 
% Ploteamos para nofz
subplot(122)
    behaviour_color = [10 10 10]/255; % Seteo el color para comportamiento   
    S_data = mov_nofz';
    y = smooth(nanmean(S_data)-8); % your mean vector;
    x = -25:0.5:25;
    stdem = smooth(nanstd(S_data,1)/sqrt(size(S_data,1)));
    curve1 = y + stdem;
    curve2 = y - stdem;
