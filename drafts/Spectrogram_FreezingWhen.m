...
SPG_freezing_CS1 = bsxfun(@minus, SPG_freezing_CS1, mean(SPG_freezing_CS1(1:end/2,:),1));
SPG_freezing_CS2 = bsxfun(@minus, SPG_freezing_CS2, mean(SPG_freezing_CS2(1:end/2,:),1));
SPG_freezing_ITI = bsxfun(@minus, SPG_freezing_ITI, mean(SPG_freezing_ITI(1:end/2,:),1));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Ploteamos duración de los bouts de freezing
% Define colors for the groups
cs1_color = [118 6 154]/255; % Color for CS+ aversive
cs2_color = [96 96 96]/255;  % Color for CS-
ITI_color = [10 10 10]/255;  % Color for ITI behavior

% Reorder the groups: 3 (ITI), 1 (CS+), 2 (CS-)
reordered_groups = freezing_when;
reordered_groups(freezing_when == 1) = 2; % CS+ -> 2
reordered_groups(freezing_when == 2) = 3; % CS- -> 3
reordered_groups(freezing_when == 3) = 1; % ITI -> 1

% Plot boxplot with reordered groups
figure;
boxplot(freezing_dist, reordered_groups, 'Colors', [ITI_color; cs1_color; cs2_color], 'Widths', 0.7, 'Symbol', '');
ylim([0 10]);
set(gca, 'XTick', [1, 2, 3], 'XTickLabel', {'ITI', 'CS+', 'CS-'});
xlim([0.5 3.5]);
ylabel('Freezing Bout Duration (sec.)', 'FontSize', 10);
set(gca, 'FontSize', 10);

% Separate the data into groups based on the original grouping
group1 = freezing_dist(freezing_when == 1); % CS+
group2 = freezing_dist(freezing_when == 2); % CS-
group3 = freezing_dist(freezing_when == 3); % ITI

% Estadística
[p1] = ranksum(group1,group2);
[p2] = ranksum(group1,group3);
...
elseif p2 < 0.01 && p2 >= 0.001 ;
    p2_value_res = '**';
elseif p2 < 0.001 && p2 >= 0.0001  ;
    p2_value_res = '***';
elseif p2 < 0.0001 && p2 >= 0.00001 ;
    p2_value_res = '****';
else
    p2_value_res = '*****';
end

text(2.5,8,p1_value_res,'HorizontalAlignment','center','VerticalAlignment','top','FontSize',10);
text(1.5,8,p2_value_res,'HorizontalAlignment','center','VerticalAlignment','top','FontSize',10);
hold off

% Set figure properties
set(gcf, 'Color', 'white');
set(gcf, 'Position', [100, 600, 200, 300]);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Ploteamos las distribuciones de tiempo
% Define the bin edges based on x-axis values
binEdges = 1:2:20;  % For example, bins from 1 to 20 with a width of 2
...
% Calculate and plot the histogram for each group
figure;

% Group 1 (CS+)
[counts1, edges1] = histcounts(group1, binEdges);
probability1 = counts1 / sum(counts1);
plot(edges1(1:end-1) + binWidth/2, probability1, '', 'Color', cs1_color, 'LineWidth', 2,'DisplayName', 'CS+');
hold on;

% Group 2 (CS-)
[counts2, edges2] = histcounts(group2, binEdges);
probability2 = counts2 / sum(counts2);
plot(edges2(1:end-1) + binWidth/2, probability2, '', 'Color', cs2_color, 'LineWidth', 2,'DisplayName', 'CS-');

% Group 3 (ITI)
[counts3, edges3] = histcounts(group3, binEdges);
probability3 = counts3 / sum(counts3);
plot(edges3(1:end-1) + binWidth/2, probability3, '', 'Color', ITI_color, 'LineWidth', 2,'DisplayName', 'ITI');
