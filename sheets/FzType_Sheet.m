%% Power quantification para detectar el tipo de freezing sin zscorear la potencia para cada uno de los eventos de EventsSheet.csv
clc
clear all
...
%                     S2 = zscorem(S,1); % Normalización por zscore
%                     S2 = S1; % Normalización quitando el pink noise
%                     S2 = 10*log10(S); % Transformación a dB.
                    S2 = bsxfun(@rdivide, S, median(S,1)); % Normalización como zscore pero con valores positivos.
...
Beta_BLA = nan(numel(ID), 1);
sGamma_BLA = nan(numel(ID), 1);
fGamma_BLA = nan(numel(ID), 1);

FourHz_PL = nan(numel(ID), 1);
Theta_PL = nan(numel(ID), 1);
Beta_PL = nan(numel(ID), 1);
sGamma_PL = nan(numel(ID), 1);
fGamma_PL = nan(numel(ID), 1);

FourHz_IL = nan(numel(ID), 1);
Theta_IL = nan(numel(ID), 1);
Beta_IL = nan(numel(ID), 1);
sGamma_IL = nan(numel(ID), 1);
fGamma_IL = nan(numel(ID), 1);
...
for i = 1:numel(ID)
    ...
    Beta_BLA(i) = nanmedian(BLA_S2{i}(Beta_idx));
    sGamma_BLA(i) = nanmedian(BLA_S2{i}(sGamma_idx));
    fGamma_BLA(i) = nanmedian(BLA_S2{i}(fGamma_idx));
    
    % PL_S2
    FourHz_PL(i) = nanmedian(PL_S2{i}(FourHz_idx));
    Theta_PL(i) = nanmedian(PL_S2{i}(Theta_idx));
    Beta_PL(i) = nanmedian(PL_S2{i}(Beta_idx));
    sGamma_PL(i) = nanmedian(PL_S2{i}(sGamma_idx));
    fGamma_PL(i) = nanmedian(PL_S2{i}(fGamma_idx));
    
    % IL_S2
    FourHz_IL(i) = nanmedian(IL_S2{i}(FourHz_idx));
    Theta_IL(i) = nanmedian(IL_S2{i}(Theta_idx));
    Beta_IL(i) = nanmedian(IL_S2{i}(Beta_idx));
    sGamma_IL(i) = nanmedian(IL_S2{i}(sGamma_idx));
    fGamma_IL(i) = nanmedian(IL_S2{i}(fGamma_idx));
end

% Create the Power_Sheet table
Power_Sheet = table(ID, ...
    FourHz_BLA, Theta_BLA, Beta_BLA, sGamma_BLA, fGamma_BLA, ...
    FourHz_PL, Theta_PL, Beta_PL, sGamma_PL, fGamma_PL, ...
    FourHz_IL, Theta_IL, Beta_IL, sGamma_IL, fGamma_IL);

Power_Sheet = sortrows(Power_Sheet, 'ID');
FzType_Sheet = Power_Sheet;

% Guardamos ambos datos
cd('D:\Doctorado\Analisis\Sheets');
% save('PSD_Sheet2.mat','PSD_Sheet','column_names','f');
% writetable(FzType_Sheet, 'Power_Sheet1.csv'); % Con zscore
% writetable(FzType_Sheet, 'Power_Sheet2.csv'); % Quitando el pink noise y normalizando a la mediana de la mediana
% writetable(FzType_Sheet, 'Power_Sheet3.csv'); % En dB
writetable(FzType_Sheet, 'Power_Sheet4.csv'); % Normalizando a la mediana de cada frecuencia
