%% Script para backupear los archivos R00D00_wavelets.mat
clc;
clear all;
...
for r = 1:length(R_folders)
    ...
    for d = 1:length(D_folders)
        ...
        epileptic_file = strcat(name, '_wavelet.mat');
        ...
        if exist(epileptic_file, 'file') == 2
            disp(['    File ' epileptic_file ' exists. Copying in H:/... ']);
            cd(newPath);
            copyfile(strcat(currentDir,'\',epileptic_file), newPath);
            cd(currentDir);
        else
            disp(['    File ' epileptic_file ' do not exist. Skipping action...']);
        end      
