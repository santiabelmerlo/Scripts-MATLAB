%% Script para backupear los archivos R00D00_acceleration.mat
clc;
clear all;
...
for r = 1:length(R_folders)
    ...
    for d = 1:length(D_folders)
        ...
        acceleration_file = strcat(name, '_acceleration.mat');
        ...
        % Copiamos R00D00_acceleration.mat
        if exist(acceleration_file, 'file') == 2;
            disp(['    File ' acceleration_file ' exists. Copying in H:/... ']);
            cd(newPath);
            copyfile(strcat(currentDir,'\',acceleration_file), newPath);
            cd(currentDir);
        else
            disp(['    File ' acceleration_file ' do not exist. Skipping action...']);
        end      
