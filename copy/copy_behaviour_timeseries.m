...
for r = 1:length(R_folders)
    ...
    for d = 1:length(D_folders)
        ...
        behaviour_timeseries_file = strcat(name, '_behavior_timeseries.mat');
        ...
        if exist(behaviour_timeseries_file, 'file') == 2;
            disp(['    File ' behaviour_timeseries_file ' exists. Copying in H:/... ']);
            cd(newPath);
            copyfile(strcat(currentDir,'\',behaviour_timeseries_file), newPath);
            cd(currentDir);
        else
            disp(['    File ' behaviour_timeseries_file ' do not exist. Skipping action...']);
        end      
